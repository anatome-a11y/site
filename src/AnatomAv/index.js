...
import { onSave as onSaveAnatomAv } from '../AnatomAv/utils';
...
class AnatomAv extends Component {
        ...
        return (
            <div style={{ padding: 24 }}>
                ...
                <Collapse bordered={false} defaultActiveKey={['geral', 'pecaFisica', 'mapeamento']} >
                    <Panel className='anatome-panel' header={<Header loading={loading} error={this.checkError(['nome', 'roteiro', 'instituicao', 'titulo', 'subtitulo', 'instrucoes', 'turma', 'disciplina'])} contentQ={<p>...</p>} title="Informações gerais da Avaliação" />} key='geral'>
                        <FormGeral
                            {...model}
                            {...options}
                            erros={erros}
    ...
    onSubmit = () => {
        onSaveAnatomAv(this.props.onOpenSnackbar, this.props.onSetAppState, this.state.model, ret => {
            this.props.onOpenSnackbar(`A avaliacao ${this.state.model.nome} foi salva com sucesso!`, 'success');
            this.props.onPush('/')
        })
    }
...
export default withAppContext(AnatomAv)